...
몇 달을 무인으로 도는 것을 전제로 한 장치들이다.

### 사이클 직렬화

스케줄러의 `max_instances` 는 **잡별**이라, 09:10 선정과 5분 간격 모니터링이 같은 엔진에서
동시에 돌 수 있었다. 그러면 `reload()` 가 스캔 도중 브로커를 교체하고, 선정이 낡은 포지션
스냅샷으로 사이징하는 사이 모니터링이 그 포지션을 청산한다.

모든 매매 사이클(선정 · 모니터링 · 청산 · 수동청산 · 정합성점검)은 엔진 단위 잠금으로
직렬화된다. 대기가 발생하면 `upbit.cycle.resumed ... waited_sec=` 로 로그에 남는다.

### 기동 시 정합성 복구 (reconcile)

주문을 낸 직후 DB 기록 전에 죽거나, 사용자가 업비트 앱에서 직접 팔면 장부에만 남은
**팬텀 포지션**이 생긴다. 그대로 두면 매 사이클마다 없는 코인을 팔려고 시도한다.

스케줄러가 뜰 때마다 보유 포지션과 실제 잔고를 대조한다.

| 상황 | 처리 |
|---|---|
| 거래소 잔고 없음 | 관리 해제 (`balance_missing`) — 매도 시도 안 함 |
| 잔고 < 기록 수량 | 기록을 실제 잔고로 **축소** |
| 잔고 > 기록 수량 | **아무것도 안 함** — 초과분은 사용자 것이지 엔진 것이 아니다 |
| 장기보유로 전환됨 | 관리 해제 (`long_term_protected`) — 코인은 그대로 |

수량은 줄이기만 하고 절대 늘리지 않는다. 없던 노출이 생길 수 없다.

### 워치독

APScheduler 는 잡 예외는 스스로 견디지만, 실행기가 하드 실패하면 **HTTP 는 멀쩡히 응답하면서
아무것도 거래하지 않는** 상태가 된다. 무인 운영에서 가장 나쁜 결과다.

60초마다 스케줄러 생존을 확인해 죽어 있으면 되살린다. 대시보드에서 스케줄을 **직접 끈** 경우는
건드리지 않는다. 부활 횟수는 `/api/health` 의 `watchdog_revivals` 로 노출된다.

### 헬스체크가 거래 여부를 본다

`/api/health` 는 아래 중 하나라도 걸리면 **HTTP 503** 과 `status: degraded` 를 돌려준다.

- 스케줄이 켜져 있는데 스케줄러가 안 돌고 있음
- 모니터링이 자기 주기의 3배가 넘도록 성공하지 못함 (기동 직후 3분은 유예)
- 어떤 잡이든 3회 연속 실패

컨테이너 헬스체크가 이 응답을 읽으므로 `docker compose ps` 가 사실을 말한다. 웹서버만 살아 있고
매매가 멈춘 상태를 healthy 로 표시하지 않는다. 대시보드 헤더와 사이드바에도 경고가 뜬다.

### 정상 종료

정지 요청이 오면 진행 중인 사이클이 끝날 때까지 기다린다 (`stop_grace_period: 90s`).
주문을 낸 직후 잘리면 그 거래가 장부에서 빠지기 때문이다. 실측: 스캔 도중 정지 시 13초 대기 후
스캔을 완주하고 종료.
...
| 작업 | 내용 |
|---|---|
| 백업 | SQLite 온라인 백업 API 로 `data_store/backups/` 에 저장, 최근 7개 보관 |
| 정리 | 분석 이력 180일 · 이벤트 90일 · 스냅샷 730일 이후 삭제 |
| VACUUM | 정리 후 공간 회수 |

**거래(`trades`)와 포지션(`positions`)은 절대 삭제하지 않는다** — 금융 기록이다.
최근 운영 이력도 보호된다 (보관 기간 이내의 실행 기록은 남는다).

> 볼륨 안 백업은 **DB 손상·오삭제** 대비용이다. 볼륨 자체가 날아가는 경우까지 대비하려면
> 6장의 tar 백업을 별도 저장소에 두어야 한다.

### 리소스 한도

메모리 1GB · CPU 1.5코어로 제한된다 (실측 사용량 약 82MB). 폭주해도 호스트를 끌어내리지 않는다.
`init: true` 로 좀비 프로세스도 방지한다.

### 관측

```bash
curl -s http://127.0.0.1:8787/api/health | jq        # 상태 · 문제 목록 · 사이클 통계
docker compose ps                                    # healthy / unhealthy
docker compose logs -f --tail 100
```

`/api/health` 의 `scheduler.health.cycles` 에 잡별 마지막 성공/실패 시각, 연속 실패 횟수,
누적 실행 횟수가 들어 있다.
...
## 10. 데이터 스키마 (`data_store/upbit.sqlite`)
